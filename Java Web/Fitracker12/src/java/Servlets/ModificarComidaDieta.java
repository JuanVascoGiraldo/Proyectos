...
public class ModificarComidaDieta extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(request.getParameter("iddie") != null && request.getParameter("idcomi") != null 
                    && request.getParameter("horario") != null && sesion.getAttribute("id") != null){
                Pattern pattern = Pattern.compile(NUMEROS);
                Matcher matcher = pattern.matcher(request.getParameter("iddie"));        
                boolean r = matcher.matches();
                pattern = Pattern.compile(NUMEROS);
                matcher = pattern.matcher(request.getParameter("idcomi"));        
                ...
                if(r == true && r2==true && r3 ==true && ((Integer)sesion.getAttribute("permiso")) == 1){
                    id_die = Integer.parseInt(request.getParameter("iddie"));
                    id_comi = Integer.parseInt(request.getParameter("idcomi"));
